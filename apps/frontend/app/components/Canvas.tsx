...
export function Canvas({
  ...
  useEffect(()=>{
    async function adminPanel(){
        ...
        //@ts-ignore
        if(response.data.response.adminId===adminIdLS){
            setadminRoom(true)
        }else{
            setadminRoom(false)
        }
  ...
  return (
    ...
    ${
     ...
     {isshare? <div className="h-screen w-screen flex items-center justify-center bg-gray-700 bg-opacity-50 fixed top-0 ">
        <div className=" h-24 w-[350px] rounded-lg  flex items-center justify-center bg-black">
          <div
            onClick={() => {
              setIsShare(false);
            }}
            className="relative text-white -top-8 cursor-pointer left-[310px] text-sm  "
          >
            <ImCross />
          </div>
          <div className="h-10 border-2 border-white  w-[300px] rounded bg-slate-900 text-white flex px-3 items-center justify-between">
            {roomId}{" "}
            <span
              onClick={async () => {
